...
export async function submitGrades(gradesData) {
  try {
    ...
    const isValid = gradesData.every(grade => grade.score >= 0 && grade.score <= 20);

    if (!isValid) {
      return { success: false, error: "فشل الحفظ: هناك درجات تتجاوز الحد الأقصى (20)" };
    }
    // استخدام insertMany لإدخال درجات الفصل بالكامل بضغطة واحدة
